...
//    fn.AppendDir(wxT("weatherfiles_pi"));
//#else
//    fn.SetPath(*GetpSharedDataLocation());
    //const char *sPluginName = "weatherfiles_pi";
    //fn.SetPath(GetPluginDataDir(sPluginName));
    //const char *sPluginName = "weatherfiles_pi";
    fn.SetPath(GetPluginDataDir("weatherfiles_pi"));
    //    fn.AppendDir( wxT("plugins") );
//    fn.AppendDir(wxT("weatherfiles_pi"));
    fn.AppendDir(wxT("data"));
    g_SData_Locn = new wxString(fn.GetFullPath().c_str());
//#endif
    wxString s = _("weatherfiles_pi data location");
    wxLogMessage( wxT("%s: %s"), s.c_str(), fn.GetFullPath().c_str());

    m_failedBitmapLoad = false;

#ifdef PLUGIN_USE_SVG
    fn.SetFullName(wxT("weatherfiles.svg"));
    m_s_weatherfiles_pi = fn.GetFullPath();
    m_bm_weatherfiles_pi = LoadSVG( fn.GetFullPath() );
    fn.SetFullName(wxT("weatherfilesgrey.svg"));
    m_s_weatherfiles_grey_pi = fn.GetFullPath();
    m_bm_weatherfiles_grey_pi = LoadSVG( fn.GetFullPath() );
    fn.SetFullName(wxT("weatherfiles-toggled.svg"));
    m_s_weatherfiles_toggled_pi = fn.GetFullPath();
...
wxBitmap tpicons::LoadSVG( const wxString filename, unsigned int width, unsigned int height )
{
    ...
    wxString s = _("weatherfiles_pi LoadSVG");
...
bool tpicons::ScaleIcons()
{
    ...
    //m_bm_weatherfiles_pi = ScaleIcon( m_p_svgd_weatherfiles_pi, m_p_img_weatherfiles_pi, m_dScaleFactor );
    //m_bm_weatherfiles_grey_pi = ScaleIcon( m_p_svgd_weatherfiles_grey_pi, m_p_img_weatherfiles_grey_pi, m_dScaleFactor );
...
void tpicons::ChangeScheme(void)
{
    switch(m_ColourScheme) {
        ...
        case PI_GLOBAL_COLOR_SCHEME_DAY:
            m_bm_weatherfiles_grey_pi = m_bm_day_weatherfiles_grey_pi;
            break;
        case PI_GLOBAL_COLOR_SCHEME_DUSK:
            m_bm_weatherfiles_grey_pi = m_bm_dusk_weatherfiles_grey_pi;
            break;
        case PI_GLOBAL_COLOR_SCHEME_NIGHT:
            m_bm_weatherfiles_grey_pi = m_bm_night_weatherfiles_grey_pi;
            break;
        default:
            break;
    }
...
{
    m_bm_day_weatherfiles_grey_pi = m_bm_weatherfiles_grey_pi;
    m_bm_day_weatherfiles_toggled_pi = m_bm_weatherfiles_toggled_pi;
    m_bm_day_weatherfiles_pi = m_bm_weatherfiles_pi;
    m_bm_dusk_weatherfiles_grey_pi = BuildDimmedToolBitmap(m_bm_weatherfiles_grey_pi, 128);
    m_bm_dusk_weatherfiles_pi = BuildDimmedToolBitmap(m_bm_weatherfiles_pi, 128);
    m_bm_dusk_weatherfiles_toggled_pi = BuildDimmedToolBitmap(m_bm_weatherfiles_toggled_pi, 128);
    m_bm_night_weatherfiles_grey_pi = BuildDimmedToolBitmap(m_bm_weatherfiles_grey_pi, 32);
    m_bm_night_weatherfiles_pi = BuildDimmedToolBitmap(m_bm_weatherfiles_pi, 32);
    m_bm_night_weatherfiles_toggled_pi = BuildDimmedToolBitmap(m_bm_weatherfiles_toggled_pi, 32);
