...
PROD_REPO=${CLOUDSMITH_PROD_REPO:-'opencpn/weatherfiles-prod'}
BETA_REPO=${CLOUDSMITH_BETA_REPO:-'opencpn/weatherfiles-beta'}
ALPHA_REPO=${CLOUDSMITH_ALPHA_REPO:-'opencpn/weatherfiles-alpha'}
...
if [ -n "${OCPN_TARGET}" ]; then
    tarball_name=weatherfiles_pi-0.1.0.0-${PKG_TARGET}-arm64-x86_64-${PKG_TARGET_VERSION}-${OCPN_TARGET}-tarball
else
    tarball_name=weatherfiles_pi-0.1.0.0-${PKG_TARGET}-arm64-x86_64-${PKG_TARGET_VERSION}-tarball
fi
...
if [ "$BUILD_BRANCH_LOWER" = "master" ]; then
    ...
    if [ -n "$BUILD_TAG"  ]; then
        ...
        VERSION="0.1.0.0+${BUILD_ID}.${commit}"
...
# Only upload if the owner did the pull request or a member of $COLLABORATORS.
    if [ "$CIRCLE_PROJECT_USERNAME" = "$CIRCLE_USERNAME" ] || [[ -n "${collab_users+1}"  && ! -z ${CIRCLE_USERNAME}  && "$collab_users" =~ "$git_user" ]]; then
        cloudsmith push raw --republish --no-wait-for-sync \
            --name weatherfiles_pi-0.1.0.0-darwin-arm64-x86_64-26.5-${OCPN_TARGET}-metadata \
            --version ${VERSION} \
            --summary "weatherfiles opencpn plugin metadata for automatic installation" \
            $REPO $xml

        cloudsmith push raw --republish --no-wait-for-sync \
            --name $tarball_name \
            --version ${VERSION} \
            --summary "weatherfiles opencpn plugin tarball for automatic installation" \
            $REPO $tarball

        if [ "${PKG_EXT}" != "gz" ] && [ "$pkg" != "" ]; then
            cloudsmith push raw --republish --no-wait-for-sync \
                --name opencpn-package-weatherfiles-0.1.0.0-darwin-arm64-x86_64-26.5-${OCPN_TARGET}.${PKG_EXT} \
                --version ${VERSION} \
                --summary "weatherfiles .${PKG_EXT} installation package" \
                $REPO $pkg
        fi
    fi
fi
